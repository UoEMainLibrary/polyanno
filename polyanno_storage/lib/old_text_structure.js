...
var votingInfoTexts = function(targetID, textArray, res) {
    ...
    var spanID = parts[1];
    var parent_id = parentID.split("transcriptions/")[1];
    setup.transcription_model.findOne({'_id': parent_id}, function(err, textParent){
        if (err) { 
            res.json({list: bracketedArray(textArray, res)});
        }
        else {
            //console.log("so the _id is "+parent_id+" and textParent is "+textParent);
...
exports.updateOne = function(req, res) {
    ...
    console.log("to be updated "+req.params.transcription_id);
    ...
    updateDoc.exec(function(err, transcription) {
        ...
        console.log("the transcription found to be updated is "+JSON.stringify(transcription));
